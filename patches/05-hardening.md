...
here is required for the multiplexer to work.
...
`":" + port` binds **all interfaces**. Under the multiplexer, every running
...
Under the multiplexer this gets more likely, not less: the admin export path
...
instance's own directory — which under the multiplexer is the tidier choice
...
obvious next line, and it is deliberately absent — the multiplexer's supervisor
...
laptop, one app", which is exactly the direction the multiplexer points.
