...
wrong under muxxerr, where the same origin hosts many instances and the
...
+  // muxxerr, whose sentinel base is rewritten per user at serve time.
...
- **Inject a `<base>` tag or a global `window.__MUX_BASE__`.** Works, but it
  makes the app depend on muxxerr to run at all, and it does not reach
