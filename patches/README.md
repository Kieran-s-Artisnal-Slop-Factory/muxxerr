...
**None of these are applied. The multiplexer works with workoutt and readerr
exactly as they are shipped today.** This directory is a set of small, optional
changes to the *apps* that make the multiplexer's job cleaner, cheaper and less
...
works, it is tested, and it is the reason you can point the multiplexer at an
...
Patch 05 is independent of the rest and independent of the multiplexer; it is
...
Then rebuild through the multiplexer, which is the only build that matters
...
Go backend at the origin root (no `--base`), and for the multiplexer.
