...
Cache Storage is keyed by **origin**, not by scope. Under muxxerr both
...
+// not by service-worker scope, so under a muxxerr (or any deployment that
...
That is not a new problem introduced by muxxerr — it is the general
...
point a dev frontend at muxxerr's port.
...
+    // muxxerr serves several under one host), and dropping their caches
