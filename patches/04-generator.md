...
+  // '/<user>/${app}' behind a multiplexer that mounts many instances on one
...
+  // multiplexer) — match anywhere in the path, not just at the root. A
...
```ts
/**
 * mux.json — this app's apps.json entry for the local-multiplexer, ready to
...
The generator produces a zip. It has no access to the multiplexer's checkout,
